...
### （2）基于zk-STARK与BLS的药企供应链产品信息追溯服务方案
...
我们提出基于zk-STARK与BLS的药企供应链产品信息追溯服务方案。在本方案中，只有拥有溯源码的人（供应链节点、最终消费者、政府部门）才能对医药产品进行溯源。各个中间环节只有提供自己拥有溯源码的证明才能对供应链进行增加环节的操作，同时又不暴露关于溯源码的任何信息。另一方面，上传加密数据时需要先对其进行签名，以此在防止篡改的同时保证医药产品的来源与去路真实可靠。同时，供应链的各个节点还需要验证前一个节点的数据，并为其生成签名，进一步提高供应链的鲁棒性。

本方案的优点在于：

1. 使用了zk-STARK（Zero-Knowledge Scalable Transparent ARguments of Knowledge，零知识的可扩展的透明的知情证明）协议，无需可信初始化，同时保证了后量子安全，更适合区块链这个去中心化的环境；
