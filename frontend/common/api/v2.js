...
export class API {
    ...
    async getTraceSignature(id) {
        return await this.client.call(
            addresses.trace,
            'function get_trace_signature(string memory id) public view returns (string memory, string memory)',
            [id],
        );
    }

    async setTrace(id, c, prev_sig, curr_sig, proof) {
        await this.client.sendRawTransaction(
            addresses.trace,
            'function set(string memory id, string memory c, string memory prev_sig, string memory curr_sig, string memory proof) public',
            [id, c, prev_sig, curr_sig, proof],
        );
    }
    ...
    async getPK(address) {
        const [pk] = await this.client.call(
            addresses.ca,
