...
1. 用户生成*新的*PRE公私钥对，将公钥发送给医生
2. 医生生成对称密钥
3. 医生使用对称密钥加密病历
4. 医生将加密后的病历上传至IPFS，获得CID
5. 医生使用用户的PRE公钥加密对称密钥
6. 医生向用户发送CID与加密后的对称密钥
7. 用户保存CID与对应的PRE私钥
8. 用户以CID生成链上id，并以链上id、加密后的对称密钥调用合约
9. 合约判断链上id是否存在
    * 若存在，则throw
    * 否则保存链上id与加密后的对称密钥
...
1. 用户根据保存的CID，生成链上id
2. 用户以链上id调用合约，获取对应的加密后的对称密钥
3. 用户以CID从IPFS获得加密后的病历
4. 用户以PRE私钥解密对称密钥，以对称密钥解密病历

#### 用户授权医生Read

下述过程中，合约不能追踪用户授权了什么数据给谁，进而保护了用户的隐私（匿名性）。

1. 医生生成PRE公私钥对，将公钥发送给用户
2. 用户使用待授权的CID对应的PRE私钥与医生的PRE公钥生成重加密密钥
3. 用户根据CID，生成链上id，并以链上id、重加密密钥调用合约
4. 合约使用重加密密钥重加密链上id对应的加密后的对称密钥，返回重加密后的对称密钥
5. 用户将CID与重加密后的对称密钥发送给医生
6. 医生以CID从IPFS获得加密后的病历
7. 医生以PRE私钥解密对称密钥，以对称密钥解密病历
...
1. 填写相关信息data，如：
  ...
  * etc.
2. 以自己的私钥为数据签名，以供后续环节验证：`sig = Sign(sk, Hash(data))`
3. 加密：`c = Enc(key, { data, sig })`
4. 生成证明，以在不泄露key的前提下证明自己拥有key：`proof = Prove(id = Hash(key))` 
   * 如果必要，可以加入当前区块高度作为nonce，以防止重放
5. 以id、c、proof调用合约

合约：
1. 验证proof：`isValid = Verify(proof, id)`
2. 若isValid为true，则`drugs[id].push(c)`，否则throw

#### 查询供应链

某个环节的参与者、患者：
1. 以id调用合约，获取`drugs[id]`
2. 解密：`drugs[id].map(c => { data, sig } = Dec(key, c))`
3. 必要时可以从CA（假设是某个具有公信力的组织）获取参与者对应的公钥，验证真实性
