...
7. 用户保存CID与对应的PRE私钥
8. 用户以CID生成链上id，并以链上id、加密后的对称密钥、时间等参数生成证明
8. 用户以证明与其它参数调用合约
9. 合约验证证明
    * 若验证失败，则throw
    * 若验证成功，则保存链上id与加密后的对称密钥
...
3. 用户根据CID，生成链上id，并以链上id、重加密密钥、时间等参数生成证明
4. 用户以证明与其它参数调用合约
5. 合约验证证明
    * 若验证失败，则throw
    * 若验证成功，则使用重加密密钥重加密链上id对应的加密后的对称密钥，返回重加密后的对称密钥
6. 用户将CID与重加密后的对称密钥发送给医生
7. 医生以CID从IPFS获得加密后的病历
8. 医生以PRE私钥解密对称密钥，以对称密钥解密病历
